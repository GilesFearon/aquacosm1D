...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!                          
       ...
       logical,                       intent(in   ) :: lin_eos,Neu_bot
...
! local variables 
       ...
       CASE(5)
          ...
          ghat(0:N) = 0.d0 
       CASE DEFAULT
          call  gls_stp (Hz,u,v,bvf,turb,Akv,Akt,r_D,sustr,svstr,      &            ! meridional wind stress  [m2/s2 = (N/m2) / (kg/m3)]
...
!-----------
          if (itrc.eq.1) then             
             ...
             do k=N-1,1,-1          
               FC(k)=srflx*swr_frac(k)-stflx(1)*ghat(k)    !<-- penetration of solar heat flux
             enddo
          else
             FC(N)=stflx(itrc)      !<-- salinity (fresh water flux)
             do k=N-1,1,-1
                 FC(k)=-stflx(itrc)*ghat(k)
                 FC(k)=0.D0                    
             enddo
          endif  
!
