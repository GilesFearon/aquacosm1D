        ...
        for mld in mlds:
            for Qswmax in Qswmaxs:  
                for p in ps:
                    ...
                    fname_out='aquacosm_p'+"{0:1.0e}".format(Diffuse.p)+'_'+react_params.Name+'_mean'+str(mean_tau)+"_amp"+str(amplitude)+"_mld"+str(mld)+"_flx"+str(Qswmax)+'.nc'
                    ...
                    ds = xr.Dataset(data_vars=dict(depth=(["time","depth_rank"],Pstore[:,:,1]),
                                                           chl_1=(["time","depth_rank"],Pstore[:,:,2]*React.Chl_C), # converting to Chlorophyll concentration for output
                                                           chl_2=(["time","depth_rank"],Pstore[:,:,3]*React.Chl_C), # converting to Chlorophyll concentration for output
                    ...
                    ds.chl_1.attrs['long_name'] = 'chlorophyll population 1'
                    ds.chl_1.attrs['units'] = 'mg m^-3'
                    ds.chl_2.attrs['long_name'] = 'chlorophyll population 2'
                    ds.chl_2.attrs['units'] = 'mg m^-3'
