...
def plot_generic(ax,n,time,z,data,label,max_val,cmap):
...
def do_the_plot(mld,amplitude,mean_tau,Qswmax,React,p):
    ...
    max_chl=12
    cmap=cm.viridis
    plot_generic(ax,0,time_aqc_plt,z_aqc,chl_aqc,'Chl mg m$^{-3}$)',max_chl,cmap)
    plot_generic(ax,1,time_diag_plt,z_diag_plt,stdev_chl,'$\sigma_{Chl}$ (mg m$^{-3}$)',8,cmap)
    plot_generic(ax,2,time_aqc_plt,z_aqc,r,'R (days$^{-1}$)',0.8,cmap)
    plot_generic(ax,3,time_diag_plt,z_diag_plt,stdev_r,'$\sigma_{R}$ (days$^{-1}$)',0.25,cmap)
    # add the thermocline
    for n in range(0,4):
