...
ps = [2.e-8] # aiming to compare with 1.e-7 using dt = 5s, as dt = 1s we now have p = 1e-7/5 = 2e-8
        ...
        for mld in mlds:
            for Qswmax in Qswmaxs:  
                for p in ps:
                    ...
                    fname_out='aquacosm_p'+"{0:1.0e}".format(Diffuse.p)+'_'+type(React.current_model).__name__+'_'+'mean'+str(mean_tau)+"_amp"+str(amplitude)+"_mld"+str(mld)+"_flx"+str(Qswmax)+'_dt1.nc'
