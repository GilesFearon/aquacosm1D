...
def do_the_plot(crocofile,eulfile,aqcfile):
    ...
    plot_C(ax,0,time_croco,z_croco,u_croco,'CROCO',t,cm.RdBu,-0.4,0.4,'u (m s$^{-1}$)')
    # plot_C(0,time_croco,z_croco,chl_croco,'CROCO',t)
    plot_C(ax,1,time_eul,z_eul,chl_eul,'Eulerian offline',t,cm.viridis,0,1,"Chlorophyll  (mg m$^{-3}$)")
    plot_C(ax,2,time_aqc,z_aqc,chl_aqc,'Aquacosms, p = '+p,t,cm.viridis,0,1,"Chlorophyll  (mg m$^{-3}$)")
    # add the thermocline
    for n in range(0,3):
